...
if [ "$VERBOSE" = "yes" ]; then
    dotnet build "$CORE_PROJECT_PATH/ModularGodot.Core.csproj" -c "$CONFIGURATION" --force --no-restore
else
    dotnet build "$CORE_PROJECT_PATH/ModularGodot.Core.csproj" -c "$CONFIGURATION" --force --no-restore > /dev/null 2>&1
fi
...
# 复制核心 DLL 到收集目录
CORE_OUTPUT_PATH="$CORE_PROJECT_PATH/bin/$CONFIGURATION/net9.0"
...
echo "🔄 步骤 6: 生成 NuGet 包..."

cd "$CORE_PROJECT_PATH"

# 更新 nuspec 文件以包含合并的 DLL
if [ -f "ModularGodot.Core.nuspec" ]; then
    # 备份原始文件
    cp ModularGodot.Core.nuspec ModularGodot.Core.nuspec.backup
    
    # 替换文件列表以只包含合并的 DLL
    sed -i 's/<file src="bin\/Release\/net9.0\/ModularGodot.*.dll" target="lib\/net9.0\/" \/>/<file src="bin\/Release\/net9.0\/ModularGodot.Core.Merged.dll" target="lib\/net9.0\/ModularGodot.Core.dll" \/>/' ModularGodot.Core.nuspec
fi

if [ "$VERBOSE" = "yes" ]; then
    dotnet pack -c "$CONFIGURATION" --no-build --output "$PACKAGE_OUTPUT_DIR"
else
    dotnet pack -c "$CONFIGURATION" --no-build --output "$PACKAGE_OUTPUT_DIR" > /dev/null 2>&1
fi
...
# 恢复原始 nuspec 文件
if [ -f "ModularGodot.Core.nuspec.backup" ]; then
    mv ModularGodot.Core.nuspec.backup ModularGodot.Core.nuspec
fi
...
# 显示结果
echo "✅ 自动化构建和合并完成!"
for PACKAGE in "$PACKAGE_OUTPUT_DIR"/*.nupkg; do
    if [ -f "$PACKAGE" ]; then
        PACKAGE_NAME=$(basename "$PACKAGE")
        PACKAGE_SIZE=$(du -m "$PACKAGE" | cut -f1)
        echo "  生成的包: $PACKAGE_NAME (大小: $PACKAGE_SIZE MB)"
    fi
done

echo "NuGet 包输出: $PACKAGE_OUTPUT_DIR"

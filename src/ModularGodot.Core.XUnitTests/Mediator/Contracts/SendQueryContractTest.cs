...
namespace ModularGodot.Core.XUnitTests.Mediator.Contracts
{
    ...
    public class SendQueryContractTest : TestBase
    {
        ...
        public async Task SendQuery_ShouldRouteToCorrectHandler()
        {
            ...
            var query = new ContractTestQuery { Number = 42 };
        ...
        public async Task SendQuery_WithCancellation_ShouldRespectCancellation()
        {
            // Arrange
            var dispatcher = ResolveService<IDispatcher>();
            var query = new ContractTestQuery { Number = 100 };
        ...
        public async Task SendQuery_WithInvalidQuery_ShouldThrowHandlerNotFoundException()
        {
            // Arrange
            var dispatcher = ResolveService<IDispatcher>();
            var invalidQuery = new InvalidContractQuery { Number = -1 };
    ...
    public class ContractTestQuery : IQuery<int>
    {
        public int Number { get; set; }
    }

    public class ContractTestQueryHandler : IQueryHandler<ContractTestQuery, int>
    {
        public Task<int> Handle(ContractTestQuery query, CancellationToken cancellationToken)
        {
    ...
    public class InvalidContractQuery : IQuery<int>
    {
        public int Number { get; set; }
    }
}